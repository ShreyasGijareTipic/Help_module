...
const AssignPage = () => {
  ...
  const [commentIsUser, setCommentIsUser] = useState(false);
  ...
  const handleAction = (newStatus) => {
    const ticketDetailsPayload = {
      ...
      status: newStatus,
      comment_is_user: commentIsUser ? 1 : 0,
    };
    ...
    axios
      ...
      .then(() => {
        ...
        alert(`Status updated to ${newStatus === 1 ? "Assigned" : "Closed"}`);
        navigate("/TicketTable");
      })
      .catch(() => {
        alert("Failed to save details or update status. Please try again.");
      });
  };
  ...
  return (
    <div style={{ padding: "20px", fontFamily: "Arial, sans-serif" }}>
      ...
      >
        ...
        />
        <select
          value={commentIsUser ? "1" : "0"}
          onChange={(e) => setCommentIsUser(e.target.value === "1")}
          style={{
            width: "100%",
            padding: "10px",
            border: "1px solid #ddd",
        ...
        >
          ...
          <option value="1">Support Engineer</option>
        </select>
          ...
          {ticketHistory.length > 0 ? (
            ticketHistory.map((item, index) => (
              <tr key={index}>
                ...
                <td style={{ border: "1px solid #ddd", padding: "10px" }}>
                  {item.comment_is_user ? "Support Engineer" : "Admin"}
                </td>
                <td style={{ border: "1px solid #ddd", padding: "10px" }}>
